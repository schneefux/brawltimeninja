...
export interface Brawler extends IdAble {
  attribution: string;
  shortDescription: string;
  fullDescription: string;
...
export default class FandomScraper {
  ...
  private parseHealthByLevel(brawlerPage: any): Pick<Brawler, "healthByLevel"> {
  ...
  private parseGenericAttack(
  ...
  ): { attack: Attack, alt: Attack | undefined } {
    ...
    const video = ruSection ? this.parseImgAsAsset(ruSection, true) : undefined;
  ...
  private parseAttack(
    ...
    $: cheerio.CheerioAPI,
    $ru: cheerio.CheerioAPI | undefined,
  ): Pick<Brawler, "attack"|"altAttack"> {
    const attack = this.parseGenericAttack("attack", "Attack", "Атака", brawlerName, brawlerPage, $, $ru);
  ...
  private parseSuper(
    ...
    $: cheerio.CheerioAPI,
    $ru: cheerio.CheerioAPI | undefined,
  ): Pick<Brawler, "super"|"altSuper"> {
    const attack = this.parseGenericAttack("super", "Super", "Супер", brawlerName, brawlerPage, $, $ru);
  ...
  private parseHypercharge(brawlerPage: any, $: cheerio.CheerioAPI): Pick<Brawler, "hypercharge"> {
  ...
  private parseGadgets(
    ...
    for (let gadgetIndex = 0; gadgetIndex < gadgetSections.length; gadgetIndex++) {
      ...
      gadgets.push({
        ...
        name,
        description,
        asset: this.findAssetByAttr($, `GD-${brawlerName}${gadgetIndex + 1}`, "data-image-name", false)!,
  ...
  private parseStarpowers(
    ...
    for (let starpowerIndex = 0; starpowerIndex < starpowerSections.length; starpowerIndex++) {
      ...
      starpowers.push({
        ...
        name,
        description,
        asset: this.findAssetByAttr($, `SP-${brawlerName}${starpowerIndex + 1}`, "data-image-name", false)!,
  ...
  private findAssetByAttr(
    ...
    const asset = this.parseImgAsAsset(img, requiresAttribution);
  ...
  private parseImgAsAsset(img: cheerio.Cheerio<any>, requiresAttribution: boolean): Asset | undefined {
  ...
  private parseDefaultSkin(brawlerName: string, $: cheerio.CheerioAPI): Pick<Brawler, "model"> {
    return {
      model: this.findAssetByAttr($, `${brawlerName} Skin-Default`, "alt", false)!,
    };
  }

  private parseAvatar(brawlerName: string, $: cheerio.CheerioAPI): Pick<Brawler, "avatar"> {
    return {
      avatar: this.findAssetByAttr($, `${brawlerName} Portrait`, "data-image-name", false)!,
    };
  }
  ...
  private parsePins($: cheerio.CheerioAPI): { skin: Asset; pin: Pin }[] {
    ...
    for (const pin of pinsTable.find("td")) {
      ...
      const asset = this.parseImgAsAsset(img, false);
      if (asset == undefined) {
  ...
  private parseBalanceHistory(brawlerPage: any): Pick<Brawler, "history"> {
  ...
  private parseConceptArt(
    brawlerName: string,
    $: cheerio.CheerioAPI|undefined,
  ): Pick<Brawler, "conceptArt"> {
    if ($ == undefined) {
      return {
        conceptArt: [],
      };
    ...
    const conceptArt = this.parseGallery(brawlerName, "concept art", $, ["Концепт-арты", "Концепты и ранние версии"], false);
  ...
  private parseAnimations(
    ...
    const animations = this.parseGallery(brawlerName, "animations", $, ["Анимации"], true);
  ...
  async getBrawlerData(
    ...
    return {
      ...
      ...this.parseAttack(brawlerName, brawlerPage, $, $ru),
      ...this.parseSuper(brawlerName, brawlerPage, $, $ru),
      ...
      ...this.parseAnimations(brawlerName, $ru),
    };
  }
}
