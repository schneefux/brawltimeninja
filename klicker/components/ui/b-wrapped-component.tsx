...
// TODO rewrite for Vue 3 or composition API
...
export default defineComponent({
  ...
  render(h, ctx) {
    ...
    const addChildren = (vNode: any, children: any) => {
      const _addChildren = (obj: any) => {
    ...
    if (!slots.wrapper) {
      const wrappers: any[] = []
      slots.default.forEach((vNode: any, i: any) => {
    ...
    if (!slots.wrapper) {
      return warn('You must inject a template slot named "wrapper" for this component to function as intended (e.g. <template v-slot="wrapper">).')
    }
    if (!slots.wrapper.length) {
      return warn('You must inject a at least one root component/element inside the wrapper template for this component to function as intended.')
    }
