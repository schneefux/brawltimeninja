-- migrate:up
CREATE TABLE brawltime.gear_meta
(
    `trophy_season_end` DateTime,
    `brawler_trophyrange` UInt8,
    `brawler_id` UInt32,
    `brawler_name` LowCardinality(String),
    `brawler_gear_id` UInt32,
    `brawler_gear_name` LowCardinality(String),
...
ORDER BY (brawler_trophyrange, brawler_id, brawler_name, brawler_gear_id, brawler_gear_name)
SETTINGS index_granularity = 8192;


-- migrate:down
DROP TABLE brawltime.gear_meta;
